...
2.  **PDFメニューの解析**
    ...
    -   時間ベースのトリガーのタイプ: `日付ベースのタイマー` や `時間ベースのタイマー` など、要件に合わせて設定（例: `1日おき`）
3.  **次週の注文募集アナウンス**
    -   実行する関数: `triggerAnnounceNextWeekOrder`
    -   実行するデプロイ: `Head`
    -   イベントのソース: `時間主導型`
    -   時間ベースのトリガーのタイプ: `週ベースのタイマー`
    -   曜日を選択: `水曜日`
    -   時刻を選択: `午前9時 ～ 午前10時` (例: 毎週水曜日の朝に注文募集アナウンス)
4.  **ランチ注文状況のSlack通知**
    ...
    -   時間ベースのトリガーのタイプ: `週ベースのタイマー`
    -   曜日を選択: `木曜日`
    -   時刻を選択: `午前9時 ～ 午前10時` (例: 毎週木曜日の朝に注文状況を報告)
5.  **週次注文処理とオーダーカード転記**
    -   実行する関数: `triggerProcessWeeklyOrders`
    -   実行するデプロイ: `Head`
    -   イベントのソース: `時間主導型`
    -   時間ベースのトリガーのタイプ: `週ベースのタイマー`
    -   曜日を選択: `木曜日`
    -   時刻を選択: `午前11時 ～ 午後0時` (例: 毎週木曜日の11時に転記とGmail下書き作成)
6.  **注文変更の自動検知と通知**
    -   実行する関数: `triggerDetectOrderChanges`
    -   実行するデプロイ: `Head`
    -   イベントのソース: `時間主導型`
    -   時間ベースのトリガーのタイプ: `日付ベースのタイマー`
    -   時刻を選択: `午後5時 ～ 午後6時` (例: 毎日17時頃に変更検知)
