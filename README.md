...
| `VERTEX_AI_PROJECT_ID` | Vertex AI (Google Cloud) のプロジェクトID                        | `your-project-id`     |
| `VERTEX_AI_LOCATION`   | Vertex AI のロケーション（リージョン）                           | `asia-northeast1`         |
...
| `B4`  | PDF解析に使用するVertex AIのモデル名                      | `gemini-3.1-flash-lite-preview`                          |
...
| `B13` | 1〜8個注文時の単価（旧体系用）                           | `500`                                                    |
| `B14` | 9〜13個注文時の単価（旧体系用）                          | `480`                                                    |
| `B15` | 14個以上注文時の単価（旧体系用）                         | `450`                                                    |
| `B16` | 請求書メールを検索するためのGmail検索クエリ               | `from:example@example.com subject:請求書`                |
| `B17` | 請求書PDF解析用のGeminiプロンプト                         | `このPDFから請求月、合計金額、合計個数を抽出してください...` |
| `B20` | **2026/02改定: 1〜8個注文時の小盛単価**                  | `660`                                                    |
| `B21` | **2026/02改定: 1〜8個注文時の中盛単価**                  | `670`                                                    |
| `B22` | **2026/02改定: 1〜8個注文時の大盛単価**                  | `680`                                                    |
| `B23` | **2026/02改定: 9〜13個注文時の小盛単価**                 | `640`                                                    |
| `B24` | **2026/02改定: 9〜13個注文時の中盛単価**                 | `650`                                                    |
| `B25` | **2026/02改定: 9〜13個注文時の大盛単価**                 | `660`                                                    |
| `B26` | **2026/02改定: 14個以上注文時の小盛単価**                | `610`                                                    |
| `B27` | **2026/02改定: 14個以上注文時の中盛単価**                | `620`                                                    |
| `B28` | **2026/02改定: 14個以上注文時の大盛単価**                | `630`                                                    |
...
各機能の動作確認用として、GASエディタ上で実行するデバッグ関数と、ローカル環境（Node.js）で実行する自動テストの2種類が用意されています。

### 1. ローカルテスト (Node.js)

価格計算などのロジックを、GASにデプロイすることなくローカル環境で高速に検証できます。

- **実行コマンド**:
  ```bash
  npm test
  ```
- **仕組み**: `tests/mockGas.js` でGASのグローバルオブジェクトをモックし、`tests/testRunner.js` でソースコードを読み込んで実行します。
- **用途**: 複雑な計算ロジックの修正時や、新料金体系の導入時の事前検証に使用します。
