...
function detectChangesForWeek(weekdays, weekType) {
  ...
  try {
    ...
    logger.info(`=== ${weekType === 'current' ? '今週' : '次週'}の変更検知開始: ${startDate}〜${endDate} ===`);
    ...
    logger.info(`${weekType === 'current' ? '今週' : '次週'}の変更検知: 追加${changes.added.length}件, キャンセル${changes.cancelled.length}件`);
...
function compareOrderSnapshots(previousOrders, currentOrders) {
  ...
  try {
    ...
    // 変更があったものだけをフィルタ
    const quantityChangesArray = Object.values(quantityChanges).filter(item => item.before !== item.after);
    
    logger.debug(`差分検知結果: 追加${added.length}件, キャンセル${cancelled.length}件, 数量変更${quantityChangesArray.length}件`);
...
function sendSlackNotification(changeResult) {
  ...
  try {
    const detectedAt = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), DATE_FORMATS.TIMESTAMP);
