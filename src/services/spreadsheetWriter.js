...
function writeMenuDataToSpreadsheet(menuData) {
  ...
  try {
    ...
    spreadsheetService.setNumberFormat(
      MENU_SHEET_NAME,
      startRow,
      1,
      dataToWrite.length,
      1,
      DATE_FORMATS.YYYY_MM_DD_HYPHEN
    );

    logger.info(
      `${dataToWrite.length}件のメニューデータを「${MENU_SHEET_NAME}」シートに書き込みました。`
    );
    return Result.success(
      dataToWrite.length,
      `${dataToWrite.length}件のメニューデータを書き込みました`
    );
  } catch (e) {
